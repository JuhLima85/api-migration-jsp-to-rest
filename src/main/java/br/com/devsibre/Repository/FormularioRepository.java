package br.com.devsibre.Repository;

import java.util.List;

import org.springframework.data.jpa.repository.Query;
import org.springframework.data.repository.CrudRepository;
import org.springframework.data.repository.query.Param;
import org.springframework.stereotype.Repository;
import org.springframework.transaction.annotation.Transactional;

import br.com.devsibre.Dtos.CadastroDTO;
import br.com.devsibre.Model.FormularioModel;

@Repository
@Transactional
public interface FormularioRepository extends CrudRepository<FormularioModel, Long> {

	List<FormularioModel> findByNomeContainingIgnoreCase(String nome);
		
	@Query("SELECT new br.com.devsibre.Dtos.CadastroDTO(f.id_c, f.nome, f.fone, f.email, f.data, f.cep, f.logradouro, f.bairro, f.localidade, f.uf, f.membro) FROM FormularioModel f WHERE f.id = :id")
	CadastroDTO findCadastroDTOById(@Param("id") Long id);
	
	 List<FormularioModel> findByMembroTrue();
	 
	 List<FormularioModel> findByMembroFalse();
	 
	 FormularioModel findByFone(String fone);
	
}
